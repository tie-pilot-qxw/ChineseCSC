...
def main():
    ...
    if args.do_train:
        ...
        if args.max_train_steps is None:
            ...
            args.save_steps = args.max_train_steps // 4
        else:
        ...
        xlora_config = xlora.xLoRAConfig(
            config.hidden_size,
            base_model_id=args.load_model_path,
            xlora_depth=8,
            device=torch.device("cuda"),
            adapters={
                "ime" : "./lora_moe/ime/step-3752",
                "gam" : "./lora_moe/gam/step-30",
                "car" : "./lora_moe/car/step-164",
                "nov" : "./lora_moe/nov/step-298",
                "enc" : "./lora_moe/enc/step-246",
                "new" : "./lora_moe/new/step-298",
                "cot" : "./lora_moe/cot/step-26",
                "mec" : "./lora_moe/mec/step-150",
                "sig" : "./lora_moe/sig/step-29",
                "ec_law" : "./lora_moe/ec_law/step-122",
                "ec_med" : "./lora_moe/ec_med/step-264",
                "ec_odw" : "./lora_moe/ec_odw/step-56",
            },
        )
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if args.do_eval and global_step % args.save_steps == 0:
                    ...
                    if accelerator.is_local_main_process:
                        ...
                        output_model_file = os.path.join(args.output_dir, "step-%s_f1-%.2f" % (str(global_step), result["eval_f1"]))
    ...
    if args.test_on_lemon:
        ...
        for key in adapters:
            adapters[key] = os.path.join(args.load_state_dict, key)

        model = xlora.from_pretrained(args.load_state_dict,model,device=device,adapters=adapters)

...
class DataProcessorForRephrasing(DataProcessor):
    @staticmethod
    def convert_examples_to_features(examples, max_seq_length, tokenizer, verbose=True, unused_length=3):
        features = []
        for i, example in enumerate(examples):
            ...
            unused_id = tokenizer.convert_tokens_to_ids("[unused98]")

            extra_length = unused_length + len(src_ids) - len(trg_ids)

            input_ids = [tokenizer.cls_token_id] + src_ids + [unused_id for _ in range(unused_length)] + [tokenizer.sep_token_id] + [tokenizer.mask_token_id for _ in trg_ids] + [tokenizer.mask_token_id for _ in range(extra_length)] + [tokenizer.sep_token_id]
            label_ids = [tokenizer.cls_token_id] + src_ids + [unused_id for _ in range(unused_length)] + [tokenizer.sep_token_id] + trg_ids + [unused_id for _ in range(extra_length)] + [tokenizer.sep_token_id]
            attention_mask = [1] * len(input_ids)
            ref_ids = [tokenizer.cls_token_id] + trg_ids + [unused_id for _ in range(extra_length)] + [tokenizer.sep_token_id] + trg_ids + [unused_id for _ in range(extra_length)]+ [tokenizer.sep_token_id]

            assert len(input_ids) == len(label_ids)
            assert len(label_ids) == len(ref_ids)
...
def main():
    ...
    if args.test_on_lemon:
        ...
        avg = 0
        for cat in ["all","ime","gam", "car", "nov", "enc", "new", "cot", "mec", "sig", "ec_law", "ec_med", "ec_odw"]:
            if cat == "all":
                eval_examples = processor.get_test_examples(args.test_on_lemon, "test.txt")
            else:
            ...
            for batch in tqdm(eval_dataloader, leave=False):
                ...
                with torch.no_grad():
                    ...
                    src_tokens = tokenizer.convert_ids_to_tokens(src_ids[0], skip_special_tokens=False)

                    for i, st in enumerate(src_tokens):
                        if st != tokenizer.cls_token and st != tokenizer.sep_token and st != tokenizer.pad_token:
                            print(st, end="")

                    print(" ")        
                    prd_tokens = tokenizer.convert_ids_to_tokens(prd_ids[0], skip_special_tokens=False)
                    trg_tokens = tokenizer.convert_ids_to_tokens(trg_ids[0], skip_special_tokens=False)

                    for i, st in enumerate(src_tokens):
                        if st != tokenizer.cls_token and st != tokenizer.sep_token and st != tokenizer.pad_token:
                            print(trg_tokens[i], end="")
                    print(" ") 
                    for i, st in enumerate(src_tokens):
                        if st != tokenizer.cls_token and st != tokenizer.sep_token and st != tokenizer.pad_token:
                            print(prd_tokens[i], end="")
                    print(" ") 
